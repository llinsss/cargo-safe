...
contract TransportContract is ERC721, Ownable, ReentrancyGuard {
    ...
    mapping(uint256 => ShipmentTerms) public shipmentTerms;
    ...
    modifier onlyCustomerOrCarrier(uint256 tokenId) {
        require(shipments[tokenId].customer == msg.sender || shipments[tokenId].carrier == msg.sender, "Not authorized");
        _;
    }
    ...
    constructor(address initialOwner) ERC721("TransportNFT", "TNFT") Ownable(initialOwner) {}
        ...
        returns (uint256)
    {
        ...
        shipmentTerms[tokenId] = ShipmentTerms({
            valueUSD: _valueUSD,
            expectedDelivery: _expectedDelivery,
            penaltyPerDay: 0 // Set to 0 for now, can be updated later
        });
    ...
    function updateShipmentStatus(uint256 tokenId, ShipmentStatus _status, uint256 _progress)
        external
        onlyCarrier(tokenId)
        shipmentExists(tokenId)
    {
    ...
    function completeShipment(uint256 tokenId) public onlyCarrier(tokenId) shipmentExists(tokenId) {
        ...
        ShipmentTerms storage terms = shipmentTerms[tokenId];

        shipment.isCompleted = true;
        shipment.status = ShipmentStatus.Delivered;

        bool onTime = block.timestamp <= terms.expectedDelivery;
        uint256 releaseAmount = shipment.escrowAmount;

        if (!onTime && terms.penaltyPerDay > 0) {
            uint256 daysLate = (block.timestamp - terms.expectedDelivery) / 1 days;
            uint256 penalty = daysLate * terms.penaltyPerDay;
    ...
    function getShipment(uint256 tokenId) external view returns (Shipment memory, ShipmentTerms memory) {
        return (shipments[tokenId], shipmentTerms[tokenId]);
    }
